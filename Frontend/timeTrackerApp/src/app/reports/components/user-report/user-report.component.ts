    ...
    <div class="container">
      <div class="header">
        <div class="header-left">
          <mat-icon class="header-icon" color="primary">assessment</mat-icon>
            <div>
            <h1>Mi informe de tiempo</h1>
            @if (report()) {
              <p class="subtitle">{{ formatDateRange() }}</p>
            } @else {
              <p class="subtitle">Ver el análisis de tu registro de tiempo</p>
            }
          </div>
        </div>
        ...
        <mat-card>
          <mat-card-content>
            <div class="filters">
              ...
              <button mat-raised-button (click)="loadReport()">
      ...
      @if (isLoading()) {
        ...
        <div class="no-data">
          <mat-icon color="primary" style="font-size: 64px; width: 64px; height: 64px;">assessment</mat-icon>
          <h2>No hay datos disponibles</h2>
          <p>Selecciona un rango de fechas para ver tu informe de tiempo</p>
        </div>
      } @else {
        <!-- Summary Card -->
        ...
        <div class="charts-section">
          ...
          <div class="chart-container">
            <app-line-chart
              ...
              [title]="'Desglose diario de horas'"
              [label]="'Horas'"
              [isLoading]="isLoading()">
            </app-line-chart>
          </div>
        ...
        <mat-card class="table-card">
            ...
            <mat-card-title>Horas por incidencia</mat-card-title>
            <mat-card-subtitle>Desglose detallado del tiempo dedicado por incidencia</mat-card-subtitle>
          </mat-card-header>
          <mat-card-content>
...
export class UserReportComponent implements OnInit {
  ...
  public projectChartLabels = computed(() => {
    return this.report()?.projectBreakdown.map(p => p.projectName) || [];
  });
  ...
  loadReport(): void {
    ...
    this.reportsService.getUserReport(startDateStr, endDateStr).subscribe({
      next: (report) => {
  ...
  exportToCSV(): void {
    ...
    let csv = 'Issue,Project,Hours\n';

    this.sortedIssues().forEach(item => {
      csv += `"${item.issueTitle}","${item.projectName}",${(item.totalHours ?? 0).toFixed(2)}\n`;
    });
    ...
    link.setAttribute('download', `time_report_${new Date().toISOString().split('T')[0]}.csv`);
