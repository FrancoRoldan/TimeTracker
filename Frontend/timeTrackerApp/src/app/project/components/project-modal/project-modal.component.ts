  ...
  onSave(): void {
    ...
    request$.subscribe({
      next: (project) => {
        ...
        Swal.fire({
          title: 'Success!',
          text: `Project "${project.name}" ${this.isEditMode ? 'updated' : 'created'} successfully`,
          icon: 'success',
          timer: 2000,
      ...
      error: (error) => {
        ...
        let errorMessage = `Failed to ${this.isEditMode ? 'update' : 'create'} project. Please try again.`;
